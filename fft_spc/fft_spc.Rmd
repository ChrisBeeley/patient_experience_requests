...
To investigate this conclusion, the scores were weighted so that each area (forensic, mental health, and community physical health) contributed the same to the score regardless of how many responses they collected. This was achieved by finding the mean of the scores in each area for each date.

```{r}

clean_division %>% 
  group_by(period) %>% 
  summarise(AV_FFT = mean(FFT)) %>% 
  ptd_spc(value_field = AV_FFT, 
          date_field = period, 
          improvement_direction = "increase") %>%  
  ptd_create_ggplot(
    fixed_y_axis_multiple = TRUE,
...
As can be seen, a great deal of the reduction that was previously observed disappears under this analysis. It would appear that a lot of the reduction in FFT score over time was simply differences in sample composition, and particularly with less data being collected from the area that tended to produce the highest FFT scores.
...
Further, a reanalysis using weighted scores also shows that a lot of the reduction is in fact caused by changes in sample composition over time, likely due to the effect of COVID on usual methods of collecting patient experience data. 
